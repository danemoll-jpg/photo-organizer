...
- [x] **Performance: destination folder (`E:\Pics`) gets fully re-hashed on every run, even files already cataloged from prior runs.** Fixed this session. `photos` gained a `file_mtime REAL` column (`schema.sql`) plus indexes on `current_path`/`original_path`. `src/db.py::find_by_path()` looks a scanned path up by `current_path` first, then `original_path` (covers the rare copied-but-not-deleted case). `src/organize.py::_process_one()` now stats the file first and, if an existing DB row matches on path + file_size + file_mtime, skips it as already-known **without computing a hash at all** — full hash only runs for new/changed files, exactly as before. `RunStats` gained `fast_path_hits` (subset of `already_known` resolved with zero hashing) so this is directly observable in the log summary line and the dashboard's results panel.
  - **Migration/backfill approach (existing DBs, including the user's real 100k+-row one):** `init_db()` now runs a one-time migration — `ALTER TABLE photos ADD COLUMN file_mtime` if the column is missing, then backfills every existing row via a plain `stat()` on its `current_path` (metadata read only, **no file content read, no re-hash**) as long as the on-disk size still matches the recorded `file_size`; a row that fails that check (file missing/moved, or size changed) is left with `file_mtime = NULL` and will get one full hash the next time *that specific file* is scanned, not the whole library. Chose stat-only backfill specifically so **the very next real run after this fix is fast for already-cataloged files**, not one-more-full-hash-pass slow. Migration logs a `Migration: ...` line with backfilled/skipped counts. Runs automatically the next time `init_db()` is called (every `main.py` command and every dashboard run) — no separate step needed.
  - **Known pre-existing, unrelated-to-this-fix edge case, surfaced during testing:** an exact-hash duplicate found under a second path is (by existing Phase 1 policy, see "Duplicate handling" below) skipped and left in place *without* ever getting its own DB row — so it has no path record to fast-path against and will keep getting a full hash on every future rescan too. This was already true before this fix (duplicates were always re-hashed every run); this fix doesn't regress it but also doesn't resolve it. Flagging in case the user wants duplicate-original paths recorded too in a future session — not done here since it's a behavior change beyond the requested pre-check, out of scope for this fix.
  - Tested against `tests/make_sample_library.py` fixtures (isolated config/db, never touched the real `config.yaml`/`data/photo_organizer.db`): run 1 (fresh) does 8 real hashes; run 2 (full rescan) resolves 8/9 files via the fast path with **zero** hashing (the 1 exception is the known duplicate-original case above) and completes in ~6ms vs run 1's ~100ms on the tiny fixture. Also tested the migration path directly: built a DB using the pre-fix schema (no `file_mtime` column) with rows pointing at real fixture files, ran the new `init_db()` against it, confirmed the column was added and both rows backfilled correctly via `stat()` (verified against actual on-disk mtimes), and confirmed an immediate rescan fast-pathed both rows with zero hashing — i.e. confirmed the *very first* run after this fix lands is already fast for existing records, not just runs after that.
- [ ] **First real run (copy-verify-delete) was started by the user against real source folder(s), then cancelled mid-run** — cancelled specifically because it was running the old slow destination-rescan behavior. Cancel was used (stops between files, nothing partial, already-processed files stay recorded) so no work was lost or corrupted. **The performance fix above is now done and tested — ready to restart.** Per session instructions, this real run was intentionally NOT restarted by Claude — the user will restart it themselves through the dashboard (same folder(s) they were actually running against — confirm which, if unclear, before restarting).
...
- `E:\Pics` itself is a **mix** of already-organized (`YYYY/YYYY-MM`) and loose/unsorted files — not a clean slate. The tool always scans `dest_root` as an implicit source in addition to whatever's in `source_folders`, and recognizes files already sitting at their correct destination path (no-op, just a DB record — no copy/delete). **This full destination re-scan is now fast at 100k+ files** — the path+size+mtime fast-path (see above) skips hashing entirely for files nothing has touched since last recorded.
- Duplicate handling: an exact-hash duplicate found under a *second* path is **skipped and left in place**, not deleted. Phase 1 does not clean up duplicate originals — only the copy-verify-delete of the first-seen instance is destructive, and only after verification. This was a judgment call (see session summary) — flag if you'd rather duplicates get cleaned up too once verified against the already-sorted copy. **Note:** because these skipped duplicates never get their own DB row, they also don't benefit from the fast-path pre-check above and get a full hash every rescan — a separate, smaller performance detail surfaced while testing the fix, not fixed this session (see fast-path note above).
