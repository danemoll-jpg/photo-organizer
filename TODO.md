...
- [ ] Extend scanner to recognize MP4, MOV, AVI alongside existing JPG/PNG/HEIC
- [ ] Video destination: `<dest_root>/YYYY/YYYY-MM/Video/` — same year/month resolution as photos, routed into a `Video` subfolder within the month folder rather than mixed with photos
- [ ] Video date resolution needs its own metadata reader (container creation-date metadata, e.g. via `hachoir`/`pymediainfo`/`ffprobe` — NOT EXIF, that's photo-only). Chain shape mirrors Phase 1: metadata → filename pattern → filesystem date → flag unsorted
- [ ] Reuse existing copy-verify-delete, hashing/dedup, collision handling, and logging as-is — these are already format-agnostic, do not reimplement for video
- [ ] Dashboard/CLI: same scan/dry-run/run flow, not a separate mode. Photo-vs-video breakdown in the results panel is a nice-to-have, not required
- [ ] **Explicitly do NOT extend Phase 2 (captioning) or Phase 3 (face detection) to video** — different problem (frame selection), not being designed here. Don't build placeholders for it.
- [ ] Test against real video files (not just synthetic fixtures) given Phase 1's precedent of catching real bugs that synthetic tests missed
- [ ] **Checkpoint: confirm with user before this is marked done**

## ACTIVE: Investigate — was `E:\Pics\katyas phone2` ever actually processed?
This subfolder of `E:\Pics` already had its own pre-existing `YYYY\YYYY-MM` structure inside it BEFORE the tool ever touched it (confirmed by the user checking their backup — this is NOT something the organizer created, not a bug in destination-path resolution). The open question is simply whether this folder's files were ever scanned/organized by a real run, since it's physically inside `E:\Pics` and should be swept by the implicit destination-scan regardless of whether it was ever explicitly added to `source_folders`.
- [ ] Search all `logs/organize_*.log` files for any mention of `katyas phone2` (or its full path) and report back: was it touched by any run, dry or real? If so, which run(s), and what happened to the files in it (moved to top-level `E:\Pics\YYYY\YYYY-MM`, left in place as already-correctly-placed no-ops, flagged unsorted, etc.)?
- [ ] Also check whether `E:\Pics\katyas phone2` (or a parent of it) appears in `config.yaml`'s `source_folders` — not required for it to be scanned (implicit dest-scan should cover it), but relevant context for interpreting what the logs show
- [ ] This is a read-only investigation — do not reorganize or move anything in this folder as part of answering the question, just report findings
...
- Image formats: JPG, PNG, HEIC (no RAW). Video formats mixed in: MP4, MOV, AVI (see Phase 1b below — not yet organized)
