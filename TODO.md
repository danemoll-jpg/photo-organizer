...
- [x] **Progress indicator** — determinate progress bar + "N / total processed" label, driven by a `progress_cb` callback added to `run_phase1()` (optional, CLI leaves it `None` so CLI behavior is unchanged)
