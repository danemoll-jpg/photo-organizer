...
- [ ] **First real run (copy-verify-delete) was started by the user against real source folder(s), then cancelled mid-run** — cancelled specifically because it was running the old slow destination-rescan behavior (full re-hash of everything in `E:\Pics` every time) right as the path+size+mtime fast-path fix was identified as needed. Cancel was used (stops between files, nothing partial, already-processed files stay recorded) so no work was lost or corrupted — this was a deliberate "let's not waste time on the slow version" call, not a problem with the run itself. **Once the performance fix below lands, this real run should be restarted** (same folder(s) the user was actually running against — confirm which with the user, wasn't specified in this note).
- [ ] **Performance: destination folder (`E:\Pics`) gets fully re-hashed on every run, even files already cataloged from prior runs.** Currently every file in `E:\Pics` is read and hashed on every invocation to check for no-op/duplicate status — at 100k+ files this is real, avoidable I/O cost being paid repeatedly for files nothing has touched since last time. Fix: use path + file size + mtime as a cheap pre-check against the existing DB record before doing a full hash; only fully hash a file if it's new (no DB record for that path) or its size/mtime don't match the recorded values (meaning it may have actually changed). Full hash remains the ground truth for actual duplicate detection and copy-verify-delete — this only skips redundant hashing of files that are almost certainly unchanged. Apply this to the `E:\Pics` implicit-source scan and to `source_folders` scanning equally.
